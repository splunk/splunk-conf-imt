...
resource "signalfx_detector" "countdown" {
  name         = "${var.sim_prefix} Host Storage Monitoring"
  description  = "Alerts when increasing disk.utilization is projected to increase to 100 in 48 hours"
  program_text = <<-EOF
  from signalfx.detectors.countdown import countdown
  A = data('disk.utilization', filter=filter('host', '*') and filter('mountpoint', '*')).publish(label='A')
  countdown.hours_left_stream_incr_detector(stream=A, maximum_capacity=100, lower_threshold=48, fire_lasting=lasting('12m', 0.95), clear_threshold=60, clear_lasting=lasting('12m', 0.95), use_double_ewma=False).publish("${var.sim_prefix} Host Storage Monitoring")
  EOF
  rule {
    detect_label       = "${var.sim_prefix} Host Storage Monitoring"
    severity           = "Critical"
